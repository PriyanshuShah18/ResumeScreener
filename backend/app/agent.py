...
class HRExtractionService:
    ...
    def _extract_structured_output(
        ...
        if has_text:
            ...
            if llm_service.enabled:
                try:
                    # Pass the detailed prompt directly instead of generating a generic one
                    llm_json, provider = llm_service._generate_json(prompt + "\nReturn valid JSON only.", None)
                    if llm_json:
                    ...
                    logger.warning("Groq/Gemini extraction failed, using heuristic: %s", exc)

            # Step 3: Return heuristic alone if LLM failed
            elapsed = time.perf_counter() - t0
            logger.info("✅ Heuristic-only extraction in %.1fs", elapsed)
            return schema_cls(**heuristic_data)
    ...
    def job_description_prompt(self, text: str) -> str:
        ...
        return (
            ...
            "Extract: (1) core must-have skills, (2) secondary good-to-have skills, (3) domain keywords, "
            "(4) tools, (5) experience years, (6) education, and (7) responsibilities. "
    ...
    def resume_prompt(self, text: str, has_images: bool = False) -> str:
        ...
        return (
            ...
            "4. Keep descriptions ultra-short.\n"
